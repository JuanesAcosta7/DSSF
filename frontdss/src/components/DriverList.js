...
const DriverList = () => {
    ...
    useEffect(() => {
        const fetchDrivers = async () => {
            try {
                ...
                setDrivers(data); // Aquí se asigna directamente el array de conductores
            } catch (error) {
    ...
    const handleDelete = async (id) => {
        ...
        if (confirmDelete) {
            ...
            if (result) {
                setDrivers(drivers.filter(driver => driver.driverId !== id)); // Filtrar por driverId
            } else {
            ...
            <ul>
                {drivers.map(driver => (
                    <li key={driver.driverId}>
                        <Link to={`/drivers/${driver.driverId}`}>{driver.driverName}</Link> {/* Enlace al detalle del conductor */}
